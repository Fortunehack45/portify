...
const ElegantSerif: React.FC<TemplateProps> = ({ user, projects }) => {
    const getSocialIcon = (platform: SocialPlatform) => {
        const iconClass = "w-5 h-5 text-gray-500 group-hover:text-gray-900 transition-colors";
        switch (platform) {
  ...
  return (
    <div style={{ fontFamily: "'Lora', serif" }} className="bg-[#fdfcf9] text-[#3d3d3d] min-h-screen">
      <main className="container mx-auto px-4 py-16 md:py-24 max-w-3xl">
        <header className="text-center mb-16 border-b border-gray-200 pb-10">
          ...
          <div className="flex justify-center gap-2 mt-6">
            ...
            {user.email && <Button variant="ghost" size="icon" className="group" asChild><a href={`mailto:${user.email}`} aria-label="Email"><Mail className="w-5 h-5 text-gray-500 group-hover:text-gray-900 transition-colors" /></a></Button>}
          </div>
        </header>
        ...
        <section id="skills" className="mb-16">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {user.skills.map((skill) => (
              <Badge key={skill} variant="outline" className="text-base px-4 py-1 rounded-full border-gray-300 font-normal hover:bg-gray-100 transition-colors">
                {skill}
              </Badge>
            ))}
          </div>
        ...
        <section id="projects">
          ...
          <div className="space-y-12">
            {projects.map((project, index) => (
              <div key={project.id} className={`pb-8 ${index < projects.length - 1 ? 'border-b border-gray-200' : ''}`}>
                ...
                <div className="flex gap-6">
                  {project.githubLink && (
                    <Button variant="link" className="p-0 text-gray-600 hover:text-gray-900 transition-colors" asChild>
      ...
      <footer className="text-center py-8 text-sm text-gray-400 border-t border-gray-200 mt-8">Made with FolioForge</footer>
    </div>
  );
};
