...
const MinimalLight: React.FC<TemplateProps> = ({ user, projects }) => {
    ...
    const getSocialButton = (platform: string, url: string) => {
        switch (platform) {
            case 'github': return <Button variant="ghost" size="sm" asChild><a href={url} target="_blank" rel="noopener noreferrer" aria-label="GitHub" className="flex items-center gap-2"><Github className="w-4 h-4" /> GitHub</a></Button>;
            case 'linkedin': return <Button variant="ghost" size="sm" asChild><a href={url} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className="flex items-center gap-2"><Linkedin className="w-4 h-4" /> LinkedIn</a></Button>;
            case 'twitter': return <Button variant="ghost" size="sm" asChild><a href={url} target="_blank" rel="noopener noreferrer" aria-label="Twitter" className="flex items-center gap-2"><Twitter className="w-4 h-4" /> Twitter</a></Button>;
            case 'website': return <Button variant="ghost" size="sm" asChild><a href={url} target="_blank" rel="noopener noreferrer" aria-label="Website" className="flex items-center gap-2"><LinkIcon className="w-4 h-4" /> Website</a></Button>;
            default: return null;
        }
    }
